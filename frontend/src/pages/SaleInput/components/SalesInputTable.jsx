...
function SalesInputTable({ platform, channelId, handlingFeeMap }) {
    ...
    const handleChange = (index, field, value) => {
        ...
        // ✅ 只有修改数量或销售价格时才重新计算手续费
        if (field === 'salesPrice' || field === 'quantity') {
            const handlingFee = calculateHandlingFee(platform, salesPrice, quantity);
                ...
                <tbody>
                    {rows.map((row, index) => (
                        <tr key={row.id || index} className={index === confirmIndex ? 'selected-row' : ''}>
                            ...
                            <td>
                                <TextField className="small-input" type="number" value={row.handlingFee} onChange={(e) => handleChange(index, 'handlingFee', e.target.value)}
                                />
                            </td>
                            <td>
